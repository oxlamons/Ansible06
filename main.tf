...
resource "hcloud_server" "renode1" {
  count       = "1"
  name        = "oxlamons.devops.rebrain.srwx.net"
  image       = "ubuntu-18.04"
  server_type = "cx11"
  ...
  provisioner "local-exec" {
    command = "echo oxlamons.devops.rebrain.srwx.net root ${element(random_password.password.*.result, count.index)} >> login.txt"
  ...
  provisioner "local-exec" {
    command = "ansible-playbook -i '${self.ipv4_address},' -u root main.yml"
  }
}
resource "null_resource" "ansible" {
  ...
  provisioner "local-exec" {
    command = "ansible-playbook -i '${element(hcloud_server.renode1.*.ipv4_address, count.index)},' -u root main.yml"
  }
}
